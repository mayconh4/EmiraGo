...
import { experiences, guides } from '@/lib/data';
...
export default function BookingPage() {
  ...
  const guide = guides.find(g => g.id === experience?.guideId);
  ...
  if (!experience) {
    return <div className="p-20 text-center">Experience not found</div>;
  }
            ...
            <div className="grid grid-cols-7 gap-2 mb-12">
               {Array.from({length: 31}).map((_, i) => (
                 <button 
                  ...
                  className={`aspect-square flex items-center justify-center rounded-full text-[15px] font-medium transition-colors ${data.date === `May ${i+1}` ? 'bg-foreground text-white' : 'hover:bg-apple-gray'}`}
                 >
                   {i+1}
                 </button>
               ))}
          ...
          <section className="animate-in fade-in slide-in-from-bottom-4 duration-500">
            ...
            <div className="flex flex-col gap-4">
              <button 
                ...
                className={`p-6 rounded-[18px] text-left transition-all group ${data.duration === '4h' ? 'bg-foreground text-white' : 'bg-apple-gray hover:bg-foreground hover:text-white'}`}
              >
              ...
              <button 
                ...
                className={`p-6 rounded-[18px] text-left transition-all group ${data.duration === '8h' ? 'bg-foreground text-white' : 'bg-apple-gray hover:bg-foreground hover:text-white'}`}
              >
        ...
        {step === 4 && (
          <section className="animate-in fade-in slide-in-from-bottom-4 duration-500">
            ...
            <div className="bg-apple-gray rounded-[18px] p-6 flex flex-col gap-6 mb-12">
               ...
               <div className="flex justify-between border-b border-foreground/5 pb-4">
                 <span className="text-foreground/40 font-medium">Experience</span>
            ...
            <div className="flex justify-between items-center mb-12">
              ...
              <span className="text-[21px] font-bold">AED {experience.price * (data.duration === '8h' ? 2 : 1)}</span>
            </div>
        ...
        {step === 5 && (
          <section className="animate-in fade-in slide-in-from-bottom-4 duration-500 text-center">
            ...
            <div className="py-20 flex flex-col items-center gap-8">
               <button onClick={nextStep} className="w-full bg-black text-white h-[64px] rounded-[12px] flex items-center justify-center gap-2">
                  <span className="text-[20px] font-semibold"> Pay</span>
               </button>
               <button onClick={nextStep} className="text-[14px] font-medium text-foreground/40 hover:text-foreground">
